...
test.describe('Patient Booking Flow', () => {
  test('create patient → book appointment → verify dashboard', async ({ page }) => {
    ...
    await form.getByRole('button', { name: 'Add to registry' }).click();
    ...
    await apptForm.getByRole('button', { name: 'Confirm booking' }).click();
    ...
    const allApptsTile = page.locator('.stat-tile', { hasText: 'All appointments' });
    const countText = await allApptsTile.locator('.value').textContent();
